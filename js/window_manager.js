...
var WindowManager = (function () {
  ...
  function WindowManager (display) {
    ...
    this.createWindow = function createWindow (name, width, height) {
      ...
      if (height != undefined) {
        var title_height = jQuery (win_struct).find ('.modal-header').height ();
        jQuery (win_struct)
          .find ('.modal-body')
      ...
      jQuery (win_struct).find ('.modal-header')
        .mousedown (function (mousedown_event) {
          // Obtain the absolute coordinates of the display.
          var display_coords = jQuery (privates.display_element).offset ();
            
          // Compute the offset of the cursor from the top left corner of the
          // window.
          var cursor_rel_coords = jQuery (privates.windows[name].struct).offset ();
          ...
          // Drag move commences.
          jQuery (window).mousemove (function (mousemove_event) {
            var win_x = mousemove_event.pageX - display_coords.left - cursor_rel_coords.left;
            var win_y = mousemove_event.pageY - display_coords.top - cursor_rel_coords.top;

            console.log ("WindowManager: dragging window \"" + name + "\" (" +
              win_x + ", " + win_y + ")");
            wm.moveWindowTo (name, win_x, win_y);

            // Set dragging state.
            privates.dragging_active = true;
            jQuery (privates.display_element).addClass ('dragging-active');
          });
        ...
        .mouseup (function () {
          console.log ("WindowManager: drag stopped for window \"" + name + "\"");

          var was_dragging = privates.dragging_active;
          jQuery (window).unbind ('mousemove');
          
          privates.dragging_active = false;
          jQuery (privates.display_element).removeClass ('dragging-active');
        });
    ...
    this.moveWindowTo = function moveWindowTo (name, x, y) {
      ...
      var window_height = jQuery (privates.windows[name].struct).height ();
      var window_width = jQuery (privates.windows[name].struct).width ();
      var title_height = jQuery (privates.windows[name].struct)
        .find ('.modal-header').height ();
      ...
      // Log a message if we are repositioned.
      if (repositioned) {
        console.log ("WindowManager.moveWindowTo: Requested coordinates out " +
            "of bounds, constraining window");
      }
    ...
    this.getWindowPosition = function getWindowPosition (name) {
      ...
      var win_struct = privates.windows[name].struct;
      var transform_mat = jQuery (win_struct).css ('transform');
    ...
    this.setWindowContent = function setWindowContent (name, content) {
      ...
      // Set the content.
      jQuery (privates.windows[name].struct)
        .find ('.modal-body')
        .append (content);
    };
    ...
    this.setFocus = function setFocus (name) {
      ...
      for (var window_name in privates.windows) {
        if (privates.windows.hasOwnProperty (window_name)) {
          jQuery (privates.windows[window_name].struct)
            .removeClass ('wm-window-focused');
        }
      }
    ...
    this.getDisplayHeight = function getDisplayHeight () {
      return jQuery (privates.display_element).height ();
    };
    ...
    this.getDisplayWidth = function getDisplayWidth () {
      return jQuery (privates.display_element).width ();
    };
    ...
    this._render = function _render () {
      ...
      var curr_actual_width = jQuery (privates.display_element).width ();
      var curr_actual_height = jQuery (privates.display_element).height ();
      ...
      for (var i = 0; i < privates.damaged_windows.length; i++) {
        ...
        jQuery (privates.windows[window_name].struct).css (
      ...
      var win = jQuery (document.createElement ('div'))
        ...
        .append (jQuery (document.createElement ('div'))
          ...
          .append (jQuery (document.createElement ('div'))
              ...
              .append (jQuery (document.createElement ('h4'))
                  .addClass ('modal-title')))
          .append (jQuery (document.createElement ('div'))
              .addClass ('modal-body')));

      jQuery (privates.display_element).append (win);
      return win[0];
    };
    ...
    console.log ("WindowManager: starting on display \"" + display + "\"...");
    ...
    privates.display_height = jQuery (privates.display_element).height ();
    privates.display_width = jQuery (privates.display_element).width ();
